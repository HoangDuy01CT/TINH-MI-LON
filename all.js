...
function applyThemeImage(){
  ...
  document.documentElement.style.setProperty('--app-bg-image',a.image?`url("${a.image}")`:'none');
  document.documentElement.style.setProperty('--app-bg-opacity',a.image?String(a.opacity/100):'0');
  const preview=document.getElementById('themeImagePreview'),controls=document.getElementById('themeImageControls');
  if(preview){preview.classList.toggle('show',!!a.image);if(a.image)preview.style.backgroundImage=`url("${a.image}")`;}
  if(controls)controls.style.display=a.image?'block':'none';
